...
class TextHighlights:
    ...
    def custom_color_from_rgb(self, fg:tuple=(255, 255, 255), bg:tuple=(0, 0, 0) ):
        if type(fg) == tuple and type(bg) == tuple:
    ...
    def custom_color_from_hexa(self, fg:str="#FFFFFF", bg:str="#000000"):
        try:
            if fg[0] == '#' and bg[0] == '#': 
    ...
    def custom_color_from_hexa_test(self, fg=None, bg=None):
    ...
    def custom_log(self, text:str="CUSTOM", color_type:str=None, fg=None, bg=None ):
        """
        Description:
        """
        if color_type == 'rgb':
            if fg and bg is not None:
                xterm_color = self.custom_color_from_rgb(fg=fg, bg=bg)
            elif fg is None and bg is not None:
                xterm_color = self.custom_color_from_rgb(bg=bg)
            elif bg is None and fg is not None: 
                xterm_color = self.custom_color_from_rgb(fg=fg)
            else:
                xterm_color = self.custom_color_from_rgb()
            if self.wrong_type:
                return "Required RGB tuple."
            else: 
            ...
            if fg and bg is not None:
                ...
                xterm_color = self.custom_color_from_hexa(bg=bg)
            elif bg is None and fg is not None: 
                xterm_color = self.custom_color_from_hexa(fg=fg)
            else: 
                xterm_color = self.custom_color_from_hexa()
            
            if self.wrong_type:
                return "Required HEXA color code."
            else:
                return f"{xterm_color}{text} \033[0m:"
            ...
            if self.wrong_type:
                return "Required Xterm 256 Standard Integer between 0 and 255"
            else:
                return f"{xterm_color}{text} \033[0m:"
        elif color_type == 'test':
            xterm_color = self.custom_color_from_hexa_test(fg=fg, bg=bg)
            if self.wrong_type:
                return "Required hexa"
            else:
                return f"{xterm_color}{text} \033[0m:"
        else: 
            if color_type is not None:
                return "You have Entered wrong color type."
    ...
    def log_error(self, color=RED, text:str = "ERROR", color_type:str='xterm'):
        ...
        Description:
        """
        if color_type == 'rgb' and type(color) == tuple:
            xterm_color = self.rgb_to_xterm(rgb=color)
            return f"{xterm_color}{text} \033[0m:"
        elif color_type == 'hexa' and color[0] == '#':
        ...
        try: 
            if hexa[0] == '#':
                ...
                if len(hexa) == 6:
                    ...
                    print(f"{self.log_error()} HEXA code length should be equal to 6 without # symbol.")
            else: 
                print(f"Hexa code should start with # string: #000000")
        except ValueError: 
            return f"{self.log_error()} Wrong HEXA color code."
